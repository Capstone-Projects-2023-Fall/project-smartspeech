...
export default function SelectedTilesActionBar() {
  ...
  const handleSpeak = () => {
    const validTilesSounds = tiles
      .filter((tile) => Boolean(tile.sound))
      .map((tile) => tile.sound);
  ...
  return (
    <div
      ...
      data-testid={actionBarDataTestIds.container}
    >
      <div
      ...
      >
        {tiles &&
          tiles.map((miniTileInfo) => (
            <MiniTile
              image={miniTileInfo.image}
              text={miniTileInfo.text}
              key={miniTileInfo.text}
            />
          ))}
      </div>
      <button
        className="bg-green-400 p-2 rounded hover:shadow-xl"
        data-testid={actionBarDataTestIds.speakBtn}
        onClick={handleSpeak}
      >
