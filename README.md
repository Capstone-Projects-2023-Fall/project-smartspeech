...
A more *secret* file will also need to be under the name of `.env.local` which requires the following fields to enable auth.
...
Notice all the DB params. The first two will be generated by terraform. The last two are variables (`username`, `password`) set by you when running the Terraform script below.
